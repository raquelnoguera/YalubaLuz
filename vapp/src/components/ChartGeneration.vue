...
<script>
/**
 * Displays a Chart showing the power generated per energy source in 30 days based on selected date.
...
const filename = "ChartGeneration";

export default defineComponent({
    name: 'ChartGenerationComponent',
    ...
    setup () {
        ...
        // WATCHERs
        watch(fecha, () => {
            console.debug(`ChartGeneration -> fecha: ${fecha.value}`);
        ...
        function updateChartData(labels, colors, datasets) {
            ...
            console.debug(`${filename} -> ${fname} -> xlabels: ${xlabels}`);
            let chartData = {
                type: "line",
                data: {
                    labels: xlabels,
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: `Potencia generada por cada fuente de generación`
                        }
                    },
                    scales: {
                        yAxes: {
            ...
            if(!chart.value) {
                console.debug("ChartGeneration -> updateChartData() -> New Chart");
                Chart.register(...registerables);
                chart.value = new Chart(ctx, chartData);
                triggerRef(chart);
            }
            else {
                console.debug("ChartGeneration -> updateChartData() -> Chart Update");
                // Remove data
        ...
        function updateList(_fechainicio) {
            ...
            console.debug(`ChartGeneration -> updateList() -> _fechainicio: ${_fechainicio}`);
            let dates = resolveDates(_fechainicio);
            console.debug(`ChartGeneration -> updateList() -> initdate: ${dates[0]} , enddate: ${dates[1]}`);
            store.reeApi.getGeneration(dates[0], dates[1])
            .then( (response) => {
                if(typeof(response) == 'number') { // an error ocurred
                    console.error(`ChartGeneration -> updateList() -> reeApi.getHourlyPrices returned ${response}`);
                    $q.notify({
                        color: 'negative',
                        textColor: 'white',
                        icon: 'error',
                        message: `Desglose de generación no disponible.`,
                        timeout: 5000,
                        position: "center"
                        })
                    return;     // nothing to do
                }
                console.debug("ChartGeneration -> updateList() -> New data retrieved");
                let labels = [];    // e.g. "Hidráulica", "Turbinación bombeo" or "Nuclear"
                let colors = [];
                let datasets = [];  // one per label
                if(response.length > 0) {
                    response.forEach((source) => {
                        if(!source.attributes.title.includes('total')) {
                            let dataset = [];
